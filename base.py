...
def gen_data_folds(config_path, folds):
    ...
    for i in folds:
        ...
        if mapped:
            ...
            dmap = parse_dmap(orig_data_dir + 'orig_doc_map.txt')
            # the new index
            ...
            doc_mapper(trainfold_dirpath + 'orig_doc_map.txt', old_index_train, new_index_train)
            doc_mapper(testfold_dirpath + 'orig_doc_map.txt', old_index_test, new_index_test)
        else:
            # Current data has not been mapped before. Copy doc_map.txt generated by qrel_mapper directly
            copy2(trainfold_dirpath + 'doc_map.txt', trainfold_dirpath + 'orig_doc_map.txt')
            copy2(testfold_dirpath + 'doc_map.txt', testfold_dirpath + 'orig_doc_map.txt')
        ...
        test_config['query-judgments'] = testfold_dirpath + 'qrels-sampled.txt' # setting judgement file path & name
